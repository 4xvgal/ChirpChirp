        ...
        while True:
            ...
            if first_byte_val in KNOWN_CONTROL_TYPES_FROM_SENDER:
                ...
                if sequence_byte_data:
                    ...
                    log_rx_event(
                        event_type="CTRL_PKT_RECV", 
                        frame_seq_recv=sequence_num, 
                        packet_type_recv_hex=first_byte_hex
                    )
            ...
            # --- 데이터 패킷 처리 ---
                ...
                if len(content_bytes) == content_len:
                    ...
                    log_rx_event(
                        event_type="DATA_FRAME_RECV",
                        frame_seq_recv=frame_seq,
                        rssi_dbm=rssi_dbm,
                        data_len_byte_value=content_len,
                        payload_len_on_wire=len(payload_chunk)
                    )
                    ...
                    try:
                        ...
                        if payload_dict:
                            ...
                            if payload_dict.get("type") in ["dummy", "dummy_bam"]:
                                ...
                                log_rx_event(
                                    event_type="DECODE_SUCCESS_DUMMY",
                                    frame_seq_recv=frame_seq,
                                    rssi_dbm=rssi_dbm,
                                    notes=f"type: {payload_dict.get('type')}, size: {dummy_size}"
                                )
